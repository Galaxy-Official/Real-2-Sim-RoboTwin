...
class Robot:
    ...
    def _init_robot_(self, scene, need_topp=False, **kwargs):
        ...
        self.left_arm_move_axis = left_embodiment_args.get("arm_move_axis", ["neg_x", "neg_x"])[0]
        self.right_arm_move_axis = right_embodiment_args.get("arm_move_axis", ["neg_x", "neg_x"])[1]
        self.left_arm_exec_mode = left_embodiment_args.get("arm_exec_mode", ["planner_path", "planner_path"])[0]
        self.right_arm_exec_mode = right_embodiment_args.get("arm_exec_mode", ["planner_path", "planner_path"])[1]
        self.left_arm_interp_step = left_embodiment_args.get("arm_interp_step", [0.03, 0.03])[0]
        self.right_arm_interp_step = right_embodiment_args.get("arm_interp_step", [0.03, 0.03])[1]
    ...
    def init_joints(self):
        ...
        if self._use_direct_gripper_qpos("left"):
            for joint in self.left_gripper:
                joint[0].set_drive_property(stiffness=0, damping=0)
        if self._use_direct_gripper_qpos("right"):
            for joint in self.right_gripper:
                joint[0].set_drive_property(stiffness=0, damping=0)
    ...
    def _use_direct_gripper_qpos(self, arm_tag):
        joints = self.left_gripper if arm_tag == "left" else self.right_gripper
        return (
            joints is not None
            and len(joints) == 7
            and joints[0][0] is not None
            and joints[0][0].get_name() == "finger_width_joint"
        )

    def _set_gripper_qpos_direct(self, arm_tag, gripper_val):
        if arm_tag == "left":
        ...
        qvel = np.array(entity.get_qvel(), dtype=np.float64)
        real_gripper_val = gripper_scale[0] + gripper_val * (gripper_scale[1] - gripper_scale[0])

        for real_joint, multiplier, offset in joints:
            idx = joint_index[real_joint.get_name()]
            qpos[idx] = real_gripper_val * multiplier + offset
            qvel[idx] = 0.0
            real_joint.set_drive_target(qpos[idx])
            real_joint.set_drive_velocity_target(0.0)

        entity.set_qpos(qpos)
        entity.set_qvel(qvel)

    def force_nominal_state(self, left_gripper_pos: float = 1.0, right_gripper_pos: float = 1.0):
        self.move_to_homestate()
        self.left_gripper_val = left_gripper_pos
        self.right_gripper_val = right_gripper_pos

        def _apply(entity, arm_joints, homestate):
            active_joints = entity.get_active_joints()
            joint_index = {joint.get_name(): idx for idx, joint in enumerate(active_joints)}
            qpos = np.array(entity.get_qpos(), dtype=np.float64)
        ...
        _apply(self.left_entity, self.left_arm_joints, self.left_homestate)
        _apply(self.right_entity, self.right_arm_joints, self.right_homestate)
        self._set_gripper_qpos_direct("left", self.left_gripper_val)
        self._set_gripper_qpos_direct("right", self.right_gripper_val)

            
    def sync_qpos_to_drive_targets(self, arm_tag=None):
        def _sync_entity(entity, arm_joints, homestate):
            active_joints = entity.get_active_joints()
            joint_index = {joint.get_name(): idx for idx, joint in enumerate(active_joints)}
        ...
        if arm_tag in [None, "left"]:
            _sync_entity(self.left_entity, self.left_arm_joints, self.left_homestate)
            self._set_gripper_qpos_direct("left", self.left_gripper_val)
        if arm_tag in [None, "right"]:
            _sync_entity(self.right_entity, self.right_arm_joints, self.right_homestate)
            self._set_gripper_qpos_direct("right", self.right_gripper_val)
    ...
    def left_plan_grippers(self, now_val, target_val):
        if self._use_direct_gripper_qpos("left"):
            ...
            delta = target_val - now_val
            max_step = 0.02
            num_step = max(1, int(np.ceil(abs(delta) / max_step)))
            result = np.linspace(now_val, target_val, num_step + 1, dtype=np.float64)[1:]
            return {
                "num_step": int(num_step),
                "per_step": float(delta / num_step),
                "result": result,
            }
        if self.communication_flag:
    ...
    def get_current_arm_qpos(self, arm_tag):
        if arm_tag == "left":
            entity = self.left_entity
        ...
        active_joints = entity.get_active_joints()
        return np.asarray([qpos[active_joints.index(joint)] for joint in arm_joints], dtype=np.float64)

    def regularize_arm_plan_result(self, result, arm_tag, start_qpos=None):
        if result is None or result.get("status") != "Success":
            return result

        exec_mode = self.left_arm_exec_mode if arm_tag == "left" else self.right_arm_exec_mode
        interp_step = self.left_arm_interp_step if arm_tag == "left" else self.right_arm_interp_step
        if exec_mode != "linear_final":
            return result

        goal_qpos = np.asarray(result["position"][-1], dtype=np.float64)
        if start_qpos is None:
            start_qpos = self.get_current_arm_qpos(arm_tag)
        else:
            start_qpos = np.asarray(start_qpos, dtype=np.float64)

        delta = goal_qpos - start_qpos
        max_delta = float(np.max(np.abs(delta))) if delta.size > 0 else 0.0
        num_step = max(1, int(np.ceil(max_delta / max(float(interp_step), 1e-4))))
        position = np.linspace(start_qpos, goal_qpos, num_step + 1, dtype=np.float64)[1:]

        if num_step == 1:
            velocity = np.zeros_like(position)
        else:
            velocity = np.vstack((
                np.zeros((1, position.shape[1]), dtype=np.float64),
                np.diff(position, axis=0),
            ))

        regularized = deepcopy(result)
        regularized["position"] = position
        regularized["velocity"] = velocity
        regularized["status"] = "Success"
        regularized["path_mode"] = "linear_final"
        return regularized
    ...
    def set_arm_qpos_direct(self, target_position, arm_tag):
        if arm_tag == "left":
            entity = self.left_entity
            joint_lst = self.left_arm_joints
        else:
            entity = self.right_entity
            joint_lst = self.right_arm_joints
        ...
        for joint, target in zip(joint_lst, target_position):
            idx = joint_index[joint.get_name()]
            qpos[idx] = target
            qvel[idx] = 0.0
            joint.set_drive_target(target)
    ...
    def set_gripper(self, gripper_val, arm_tag, gripper_eps=0.1):  # gripper_val in [0,1]
        ...
        if self._use_direct_gripper_qpos(arm_tag):
            current_val = self.left_gripper_val if arm_tag == "left" else self.right_gripper_val
            if (gripper_val - current_val > gripper_eps
                    and gripper_eps > 0) or (gripper_val - current_val < gripper_eps and gripper_eps < 0):
                gripper_val = current_val + gripper_eps
            gripper_val = float(np.clip(gripper_val, 0, 1))
            if arm_tag == "left":
                self.left_gripper_val = gripper_val
            else:
                self.right_gripper_val = gripper_val
            self._set_gripper_qpos_direct(arm_tag, gripper_val)
            return
